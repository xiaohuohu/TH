...
BOOL	 ftoa( FP32 op1, INT8U *str, INT8U dotlen,INT16S len)
{
	...
	if(integer>0)
		fract = (op1-(FP32)integer)+0.000001;
	else
	{
		if(integer<0)
		{
	...
	integer=(INT32S)(fract*pow((double)(10.0),(double)(dotlen)));  

	str2[0] = '.';

	if(integer==0&&dotlen>0)
	{
	  	INT16S i;  	
	...
	if(len>0)
	{
	  	...
	  	if(strlen((INT8S *)str)<len)
	  	{
	  		for(i=strlen((INT8S *)str);i<len;i++)
	  			str[i]=' ';
		...
		if(len<0)
  		{
			...
			if(strlen((INT8S *)str)<tmp)
			{
	  			for(i=0;i<tmp-strlen((INT8S *)str);i++)
	  			{
	  				str2[i]=' ';
	  			}
	  			
	  			str2[i]=0;
	  			strcat((INT8S *)str2,(INT8S *)str);
...
FP64 stringtof(INT8U* str)
{
    ...
    result =(FP64)integer + fract+0.000001;
