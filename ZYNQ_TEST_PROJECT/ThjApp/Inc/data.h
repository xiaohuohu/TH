...
typedef struct
{
	...
	INT8U	iDuanXianLogic;				//断线报警逻辑（0:低电平有效  1:高电平有效）（未用）
	
	INT16S	iDuanXianNum;				//断线报警输入端口号     (未用)
	INT16S	iSongXianNum;				//缠线报警输入端口号	（未用）
	INT16S	iPaoXianNum;				//跑线报警输入端口号	 （未用）	
	...
	INT8U 	iF1;						//F1按键模式（0：气缸按键，1：设起点按键）
	...
	INT16S 	iAxisCa;					//制程精度，轴的表达精度0精度为1:1,1:精度为1:0.1
	
	INT16S  iLimitStopAxis;				//碰限位停止轴

	INT16S 	StopLevel;					//急停按钮检测电平

	INT16S  iPasswordTime;				//修改需要密码的时间

	INT8U   iBackStyle;                 //是否强制归零

	INT16S	iSafeDoor;					//安全门使能
	INT32S	iDuXianNum;					//堵线计数，在该数量内没有收到堵线信号报警
