...
void	CSetTime_Init(INT16S left,INT16S top,INT16S width,INT16S height)
{
	...
	temp.x=g_iDlgLeft+125+SETTIMEWOFFSET;
	j=g_iDlgTop+80-SETTIMEHOFFSET;
	temp.ID=0x9001+k++;
	temp.y=(j+=SETTIMEHOFFSET);
	CMainFrm_Add(i++,&temp);
	...
	temp.x=g_iDlgLeft+g_iDlgWidth/2-180;
	...
	temp.x=g_iDlgLeft+g_iDlgWidth/2;
	temp.y=g_iDlgTop+g_iDlgHeight-100;
	temp.w=150;temp.h=40;
	temp.key=0;
	temp.algin=CENTER_ALGIN;
	...
	temp.x=g_iDlgLeft+g_iDlgWidth/2-80;
...
void   CSetTime_Create(void)
{
	...
	CDC_OutText_UTF(g_iDlgLeft+(g_bEnglish?35:55),j+=SETTIMEHOFFSET,g_bEnglish?"Year":"年:",g_TextColor3,DARKGRAYCOLOR,KZK_24);
	CDC_OutText_UTF(g_iDlgLeft+(g_bEnglish?35:55),j+=SETTIMEHOFFSET,g_bEnglish?"Mon":"月:",g_TextColor3,DARKGRAYCOLOR,KZK_24);
	CDC_OutText_UTF(g_iDlgLeft+(g_bEnglish?35:55),j+=SETTIMEHOFFSET,g_bEnglish?"Day":"日:",g_TextColor3,DARKGRAYCOLOR,KZK_24);

	j=g_iDlgTop+80-SETTIMEHOFFSET;
	CDC_OutText_UTF(g_iDlgLeft+(g_bEnglish?55:78)+SETTIMEWOFFSET,j+=SETTIMEHOFFSET,g_bEnglish?"Hour":"时:",g_TextColor3,DARKGRAYCOLOR,KZK_24);
	CDC_OutText_UTF(g_iDlgLeft+(g_bEnglish?55:78)+SETTIMEWOFFSET,j+=SETTIMEHOFFSET,g_bEnglish?"  Min":"分:",g_TextColor3,DARKGRAYCOLOR,KZK_24);
	CDC_OutText_UTF(g_iDlgLeft+(g_bEnglish?55:78)+SETTIMEWOFFSET,j+=SETTIMEHOFFSET,g_bEnglish?"  Sec":"秒:",g_TextColor3,DARKGRAYCOLOR,KZK_24);
...
void	CSetTime_UpdateObject(INT16S dk)
{
	CTROL *ptr;
...
//				CBmp_Destroy();
		...
		case TENTER:
			{
				...
				else
				{
					...
					{
						CSetTime_OnButtonSetTime();
						OSQPost(MsgQueue,g_bEnglish?"Time Setting Succeed.":"时间设定成功.");
					}
					
					if(g_iDlgCurObject == SETTIMECOUNT-3)
					{
						CSetTime_OnButtonSetDate();
						OSQPost(MsgQueue,g_bEnglish?"Date Setting Succeed.":"日期设定成功.");
					}
				}
				break;
			}
		...
		case '6':case '7':case '8':case '9':case '.':case TMINUS:
		case TCLEAR:
