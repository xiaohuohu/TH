...
void	CDC_Rectangle4(INT16S x1,INT16S y1,INT16S x2,INT16S y2,INT32S color)
{
	...
	CDC_Line(x1,y1,x1,y2,color);
	CDC_Line(x2,y1,x2,y2,color);
}
...
{
	INT8S  *axisNames[15] ={"凸轮","送线","卷线","转芯","节距","卷取","夹耳","切刀","丝杆","螺距","开槽","","","",};
	static INT8S  nameBuff[0x80]="";
	static INT8S  msgBuff[0x80]="";
	INT8S  *defaultName[MAXCH]={"X","Y","Z","A","B","C"};
	
	INT8S  *englishName[MAXCH]={"X","Y","Z","A","B","C"};
