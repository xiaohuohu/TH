...
#define     ID_NUM          6
U32			g_iProductId[ID_NUM] = {0,0x64091560,0xfffffff6,0xd82080c0,0x64091180};		//0为通用机型，0x64091560为BZ001C171A项目,0xfffffff6为DXDA015007(万能820B六轴)
																				//0xd82080c0为DW004B006A(万能820四轴)0x64091180对应BZ001C140A（820B标准）项目
INT32S		g_iProductErron[ID_NUM];
...
//	memset(&g_FW,0x00,sizeof(FIRMWARE));
	...
	HwPlatform.MachineType = MT_TH9100;
...
//	DispRunTimeSem = OSSemCreate(0);
	...
	// ****************硬件初始化 ***************** //
	{
		...
		InitXMS();
		CDC_Init(6);																//参数为4是800*480  3为800*600
//		LCD_Power(1);																//1打开，0背光
...
/**************************************************************/
	...
	CDC_OutText24(300,400,"Start System ...",WHITECOLOR,BLACKCOLOR);
	...
	CDC_OutText_UTF(300,400,g_bEnglish?"Hardware initing...         ":"硬件初始化...            ",WHITECOLOR,BLACKCOLOR,KZK_24);
	...
	{
		CDC_PutColorBar(0,0,getmaxx(),getmaxy(),BLACKCOLOR,100);
		CDC_OutText_UTF(300,300,g_bEnglish?"Hardware Error!Press any key...":"硬件异常!按任意键...",WHITECOLOR,BLACKCOLOR,KZK_24);
	...
	{
		if(strcmp(PROJECT_ID,"DXDA015007")==0)
		{
			SetFCS(2);
		}
		else
