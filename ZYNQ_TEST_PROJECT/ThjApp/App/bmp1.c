...
//	Uart_Printf("H=%d,W=%d,BIT=%d\n",bmpinfo.biHeight,bmpinfo.biWidth,bmpinfo.biBitCount);
...
BOOL  DispWords(int Num,int x,int y)
{
	...
	for(h=0;h<48;h++)
		for(w=3;w>=0;w--)
		{
			ret = Words[Num*48*4+h*4+w];
			//Uart_Printf("ret = %d,w=%d,h=%d\t" ,ret,w,h);
			for(j=0;j<8;j++)
			{
				cc = 0x0000001<<j;


				CDC_PutPoint(x+w*8+j,h+y,ret&cc?MAGENTACOLOR:BLACKCOLOR);

				//ret = ret%cc;
				//Uart_Printf("cc=%d,ret=%d\n",cc,ret);
			}
		}	*/
	...
	*/
	...
	for(col=0;col<48;col++)
	{
		for(row=0;row<4;row++)
		{
			ByteVal = HzkCode[Num*48*4+col*4+row];
			for(i=0;i<8;i++)
			{
				CDC_PutPoint(x+row*8+i,col+y,ByteVal & mask[i]?PINKCOLOR:BLACKCOLOR);
			}
		}
...
BOOL  DispWordsH(int Num,int x,int y)
{
	...
	for(col=0;col<48;col++)
	{
		for(row=0;row<6;row++)
		{
			ByteVal = HzkCode[12*48*4+Num*48*6+col*6+row];
			for(i=0;i<8;i++)
			{
				CDC_PutPoint(x+row*8+i,col+y,ByteVal & mask[i]?PINKCOLOR:BLACKCOLOR);
