...
typedef struct
{
	...
	int   FirstCode;
	int   OldCode;
}__attribute__((__packed__))LZW_INFO;
...
typedef struct
{
	...
	u8 pixratio;	//像素宽高比
}__attribute__((__packed__))LogicalScreenDescriptor;
...
}__attribute__((__packed__))ImageScreenDescriptor;
...
typedef struct
{
	...
	u16 delay;					    //延迟时间
	LZW_INFO *lzw;					//LZW信息
}__attribute__((__packed__))gif89a;
