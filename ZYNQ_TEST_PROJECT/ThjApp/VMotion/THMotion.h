...
#define MaxMParaPart 		4000								//设定缓冲区长度
#define MaxEParaPart 		4000								//设定EP缓冲区长度
...
#define MAXAXIS 	6											//定义运动轴数
#define INPA		6											//第一组插补寄存器
#define INPB		7											//第二组插补寄存器
...
VM_PUBDEF  	EP 			EPDataBuf[MaxEParaPart];
...
VM_PUBDEF 	FP32 		RANGERATE1,RANGERATE2;					//
VM_PUBDEF 	INT32S 		FPGARATE;
...
int 	vm_abs_inp_move(int index,int axismap,int maxis,int pos1,int pos2,int pos3,int pos4,int pos5,int pos6,int speed);	//六轴绝对位置缓存插补
