...
value[6]卷取，value[7]夹耳，value[8]为速比，value[9]为探针，value[10]为气缸，value[11]为延时
	...
	INT32S 	lCompenstionYPos = 0;					//记录转线轴转动时，送线轴相应的补偿量
	WorkErr = TRUE;
...
Uart_Printf("lxspeed==%d  lyspeed==%d  lzspeed==%d  laspeed==%d\r\n",lxspeed,lyspeed,lzspeed,laspeed);
						...
						if(j<7)
						{
							...
							else if(j >= 3 && j <= 6 )
							{
								if(g_Ini.iAxisRunMode[j-1] == 1)
									...
									n[j] = n[j]/10.0;
								else if(g_Ini.iAxisRunMode[j-1] == 2)
									n[j] = n[j]/100.0;
							}
						...
						if(templen>0)
						{
							for( l=0;l<templen;l++)
							{
								if(ptr->value[MAXVALUECOL-2][l]=='0')
									ostatus[l]=0;
								else if(ptr->value[MAXVALUECOL-2][l]=='1')
									ostatus[l]=1;
								else
									ostatus[l]=2;
							}
...
Uart_Printf("g_UnitData.fDelay == %f\r\n",g_UnitData.fDelay);
				...
				if( lastla != la)													//当转线轴有运动时，送线轴相应补偿
				{
					//Uart_Printf("AA=%d %d\n",la,lastla);

					lCompenstionYPos = -((INT32S)((la - lastla)*(FP64)g_Ini.lRotatorRate/1000000.0));

					ly = ly + lCompenstionYPos;
...
Uart_Printf("速比n[7]：%f\r\n",n[7]);
							...
							else
							{
								...
								else
								{
									...
									else
									{
										...
										else if(bXReset == TRUE || bZReset == TRUE ||  bAReset == TRUE || bBReset == TRUE ||  bCReset == TRUE)		//若X,Y,Z数据都不变，但有数据归零情况下，加一条指令 。
										{
				...
				{													//该函数用了计数在进入加工循环前时，X,Z,A轴的位置情况，和循环中总的送线长度
					...
					for(l=0;l<loopno;l++)							//从第一行到循环起始行减1查找
					{
						...
						if(strcmp(ptr->value[0],"M") == 0)
						{
							...
							else if( ptr->value[4][0] && g_Ini.iAxisRunMode[2] == 2)
							{
							...
							else if( ptr->value[5][0] && g_Ini.iAxisRunMode[3] == 2)
							{
		...
		for( ; iNode<g_lXmsSize; )
		{
			...
			case MOVE:
				/*
				{
					...
					if(fPDly>0.0001 && pData.nReset!=0 && labs(lPValue[0]-pData.lValue[0])<2 && labs(lPValue[1]-pData.lValue[1])<2 &&labs(lPValue[2]-pData.lValue[2])<2 
						&&labs(lPValue[3]-pData.lValue[3])<2 &&labs(lPValue[4]-pData.lValue[4])<2&&labs(lPValue[5]-pData.lValue[5])<2)
					{
						g_bUnpackCorrect=FALSE;
						g_bDataCopy=FALSE;
						return 103;	
					...
					lPValue[4]=pData.lValue[4];
					lPValue[5]=pData.lValue[5];
					fPDly	  =pData.fDelay;
	...
	g_bDataCopy=FALSE;
	ps_debugout("数据格式是否错误  WorkErr = %d\r\n ",WorkErr);
