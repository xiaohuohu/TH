...
//	INT16S	j=g_iDlgTop+45-SYSHOFFSET;
...

	{
	 ...
	 //		:CSysPara_PutPara("Y送线长度","Y轴旋转一圈长度.单位:毫米.范围(1-2000).",&g_Ini.fMM[1],PARAFLOAT,1,6,0);
		...
		g_bEnglish?
	 		(CSysPara_PutPara(g_Ini.iAxisRunMode[1]==2?"YPitch":"YEddyDeg",
	 			g_Ini.iAxisRunMode[1]==2?"Pitch of Y-axis.UNIT:mm.Range(1-2000).":"Degrees per Round of Y-axis.UNIT:°.Range(1-2000).",&g_Ini.fMM[1],PARAFLOAT,2,6,0))
	 		:CSysPara_PutPara(g_Ini.iAxisRunMode[1]==2?"Y丝杆螺距":"Y旋转角度",
	 		g_Ini.iAxisRunMode[1]==2?"Y轴的螺距大小.单位:毫米.范围(1-2000).":"Y轴旋转一圈角度.单位:度.范围(1-2000).",&g_Ini.fMM[1],PARAFLOAT,2,6,0);
...
*/
	 	...
	 	g_bEnglish?CSysPara_PutPara("XGearRate","Gear Rate of X-axis.Range(0.1-1000).",&g_Ini.lGearRate[0],PARAFLOAT,1,4,0)	
	 		:CSysPara_PutPara("X减速比","X轴机器减速比.单位:比值.范围(0.1-1000).",&g_Ini.lGearRate[0],PARAFLOAT,1,4,0);
	 	
	 	g_bEnglish?CSysPara_PutPara("YGearRate","Gear Rate of Y-axis.Range(1-1000).",&g_Ini.lGearRate[1],PARAFLOAT,1,4,0)	
	 		:CSysPara_PutPara("Y减速比","Y轴机器减速比.单位:比值.范围(0.1-1000).",&g_Ini.lGearRate[1],PARAFLOAT,1,4,0);
	 	
	 	g_bEnglish?CSysPara_PutPara("ZGearRate","Gear Rate of Z-axis.Range(0.1-1000).",&g_Ini.lGearRate[2],PARAFLOAT,1,4,0)	
	 		:CSysPara_PutPara("Z减速比","Z轴机器减速比.单位:比值.范围(0.1-1000).",&g_Ini.lGearRate[2],PARAFLOAT,1,4,0);
	 	
	 	g_bEnglish?CSysPara_PutPara("AGearRate","Gear Rate of A-axis.Range(0.1-1000).",&g_Ini.lGearRate[3],PARAFLOAT,1,4,0)	
	 		:CSysPara_PutPara("A减速比","A轴机器减速比.单位:比值.范围(0.1-1000).",&g_Ini.lGearRate[3],PARAFLOAT,1,4,0);
	/* 	
...
*/
	...
	*/		
	 	...
	 	g_bEnglish?CSysPara_PutPara("YBackSpeed","BackSpeed of Y-axis.UNIT:°/s(mm/s).Range(1-1000).",&g_Ini.fBackSpeed[1],PARAFLOAT,1,6,0)
	 		:CSysPara_PutPara("Y归零速度","Y轴归零的速度.单位:度/秒(毫米/秒).范围(1-1000).",&g_Ini.fBackSpeed[1],PARAFLOAT,1,6,0);
...
*/
	 	...
	 	g_bEnglish?CSysPara_PutPara("YBackMode","BackMode of Y.0:Nearby,1:Foreword,2:Backword.3:Zero.4:No",&g_Ini.iBackMode[1],PARAINT,0,1,0)
	 		:CSysPara_PutPara("Y回零模式","Y轴加工完回零的方式.(0:就近,1:正向,2:负向,3:零向,4:不回零).",&g_Ini.iBackMode[1],PARAINT,0,1,0);
...
*/
	 	...
	 	g_bEnglish?CSysPara_PutPara("YSearchZero","0:Nope,1:Find machine zero,2:Find servo zero,3:Both",&g_Ini.iSearchZero[1],PARA8U,0,1,0)
	 		:CSysPara_PutPara("Y搜索零点方式","0:不归零,1:搜索机械零点,2:搜索伺服零点,3:搜索机械零点和伺服零点.",&g_Ini.iSearchZero[1],PARA8U,0,1,0);
	...
	*/ 		
	 	...
	 	g_bEnglish?CSysPara_PutPara("XName","0:Default,X-axis.",&g_Ini.iAxisNameNo[0],PARAINT,0,2,0)
	 		:CSysPara_PutPara("X轴名称","0默认,1凸轮,2送线,3卷线,4转芯,5节距,6卷取,7夹耳,8切刀,9丝杆",&g_Ini.iAxisNameNo[0],PARAINT,0,2,0);
	 		
	 	g_bEnglish?CSysPara_PutPara("YName","0:Default,Y-axis.",&g_Ini.iAxisNameNo[1],PARAINT,0,2,0)
	 		:CSysPara_PutPara("Y轴名称","0默认,1凸轮,2送线,3卷线,4转芯,5节距,6卷取,7夹耳,8切刀,9丝杆",&g_Ini.iAxisNameNo[1],PARAINT,0,2,0);
	 		
	 	g_bEnglish?CSysPara_PutPara("ZName","0:Default,Z-axis.",&g_Ini.iAxisNameNo[2],PARAINT,0,2,0)
	 		:CSysPara_PutPara("Z轴名称","0默认,1凸轮,2送线,3卷线,4转芯,5节距,6卷取,7夹耳,8切刀,9丝杆",&g_Ini.iAxisNameNo[2],PARAINT,0,2,0);
	 		
		g_bEnglish?CSysPara_PutPara("AName","0:Default,A-axis.",&g_Ini.iAxisNameNo[3],PARAINT,0,2,0)
	 		:CSysPara_PutPara("A轴名称","0默认,1凸轮,2送线,3卷线,4转芯,5节距,6卷取,7夹耳,8切刀,9丝杆",&g_Ini.iAxisNameNo[3],PARAINT,0,2,0);
	 /*		
	 	g_bEnglish?CSysPara_PutPara("BName","0:Default,B-axis.",&g_Ini.iAxisNameNo[4],PARAINT,0,2,0)
	 		:CSysPara_PutPara("B轴名称","0默认,1凸轮,2送线,3转芯,4转线,5卷取,6夹耳,7节距,8切刀,9上切,10下切",&g_Ini.iAxisNameNo[4],PARAINT,0,2,0);
	 		
		g_bEnglish?CSysPara_PutPara("CName","0:Default,C-axis.",&g_Ini.iAxisNameNo[5],PARAINT,0,2,0)
	 		:CSysPara_PutPara("C轴名称","0默认,1凸轮,2送线,3转芯,4转线,5卷取,6夹耳,7节距,8切刀,9上切,10下切",&g_Ini.iAxisNameNo[5],PARAINT,0,2,0);
	 */	
	...
	*/		
	 	...
	 	g_bEnglish?CSysPara_PutPara("YRunMode","RunMode of Y-axis.0:Circle,1:Circles,2:Screw.",&g_Ini.iAxisRunMode[1],PARAINT,0,1,0)
			:CSysPara_PutPara("Y轴方式","Y轴的运动类型.0-单圈方式,1-多圈方式,2-丝杆方式,3-送线方式.",&g_Ini.iAxisRunMode[1],PARAINT,0,1,0);
	...
	/*		
	 	...
	 	*/
	 	...
	 	g_bEnglish?CSysPara_PutPara("YMulRunMode","RunMode of Y-axis.0:Relatively,1:Absolute.",&g_Ini.iAxisMulRunMode[1],PARAINT,0,1,0)
	 		:CSysPara_PutPara("Y轴多圈丝杠表达","当Y轴为多圈或丝杠模式时,运行位置表达方式.0:相对,1:绝对",&g_Ini.iAxisMulRunMode[1],PARAINT,0,1,0);
	 	
	 	g_bEnglish?CSysPara_PutPara("ZMulRunMode","RunMode of Z-axis.0:Relatively,1:Absolute.",&g_Ini.iAxisMulRunMode[2],PARAINT,0,1,0)
	 		:CSysPara_PutPara("Z轴多圈丝杠表达","当Z轴为多圈或丝杠模式时,运行位置表达方式.0:相对,1:绝对",&g_Ini.iAxisMulRunMode[2],PARAINT,0,1,0);
	 		
	 	g_bEnglish?CSysPara_PutPara("AMulRunMode","RunMode of A-axis.0:Relatively,1:Absolute.",&g_Ini.iAxisMulRunMode[3],PARAINT,0,1,0)
	 		:CSysPara_PutPara("A轴多圈丝杠表达","当A轴为多圈或丝杠模式时,运行位置表达方式.0:相对,1:绝对",&g_Ini.iAxisMulRunMode[3],PARAINT,0,1,0);	
	/* 	
	...
	*/
	 	...
	 	g_bEnglish?CSysPara_PutPara("Axis Ca","axis.Capability of accuracy.0:1,1:0.1",&g_Ini.iAxisCa,PARAINT,0,1,0)
	 		:CSysPara_PutPara("轴精度","轴的表达精度.0:1度,1:0.1度",&g_Ini.iAxisCa,PARAINT,0,1,0);


	 	g_bEnglish?CSysPara_PutPara("Limit Stop ","0:invalid,(1-4) Axis can be set,default 3",&g_Ini.iLimitStopAxis,PARAINT,0,1,0)
	 		:CSysPara_PutPara("归零碰限位停止轴","0:碰限位不停止,(1-4)为碰限位停止轴号,默认3",&g_Ini.iLimitStopAxis,PARAINT,0,1,0);

	 	g_bEnglish?CSysPara_PutPara("Stop Level","0: low level, 1: high level",&g_Ini.StopLevel,PARAINT,0,1,0)
	 		:CSysPara_PutPara("急停检测电平","1高电平,0低电平",&g_Ini.StopLevel,PARAINT,0,1,0);

	 	g_bEnglish?CSysPara_PutPara("Break Level","0: low level, 1: high level",&g_Ini.iDuanXianLogic,PARA8U,0,1,0)
	 		:CSysPara_PutPara("断线报警逻辑","断线报警逻辑(0:低电平有效  1:高电平有效)",&g_Ini.iDuanXianLogic,PARA8U,0,1,0);
	 	...
	 	g_bEnglish?CSysPara_PutPara("MachineType","0:TH08R,1:TH412,2:TH416,3:CNC428,4:CNC820.",&g_Ini.iMachineType,PARAINT,0,2,0)
			:CSysPara_PutPara("机器型号","0:TH08R,1:TH412,2:TH416,3:CNC428,4:CNC820.",&g_Ini.iMachineType,PARAINT,0,2,0);
	 	...
	 	g_bEnglish?CSysPara_PutPara("F1 AsSetOrigin","0:NO,1:SetOrigin.",&g_Ini.iF1,PARA8U,0,1,0)
	 		:CSysPara_PutPara("F1设起点","F1按键当作设起点按键.0-无.1-设起点.",&g_Ini.iF1,PARA8U,0,1,0);
	...
	// 		:CSysPara_PutPara("左旋右旋","Z轴与A轴名字切换.0-左旋.1-右旋.",&g_Ini.iLeftOrRight,PARA8U,0,1,0);
		...
		g_bEnglish?CSysPara_PutPara("ForeBackZero","Fore back zero?.0:NO,1:Yes.",&g_Ini.iBackStyle,PARA8U,0,1,0)
			:CSysPara_PutPara("强制归零","0:不强制归零,1:强制归零,默认0",&g_Ini.iBackStyle,PARA8U,0,1,0);

		g_bEnglish?CSysPara_PutPara("Probe Delay"," Delay time s.",&g_Ini.iProDelay,PARAFLOAT,0,6,0)
		...
		switch(SysParaType[s_StartLine+s_CurLine])
		{
			...
			case PARAFLOAT:
				if(IsEnter)
				{
					...
					if(s_StartLine+s_CurLine==5)
						*((FP32*)p_SysPara[s_StartLine+s_CurLine])=CEdit_GetFloat()+0.00001; //特殊处理郛点型无法表达219.9等	
					else
...
void	CSysPara_OnKey(INT16U key,INT16U tkey)
{
	...
	switch(key)
	{
		...
		case TENTER:
			...
			//	if(g_iDlgCurObject < s_iSysParaCount-2)
				{
			...
			/*	else
				{
					...
					if(g_iDlgCurObject==s_iSysParaCount-1)
						CSysPara_OnButtonCancel();
				}*/
