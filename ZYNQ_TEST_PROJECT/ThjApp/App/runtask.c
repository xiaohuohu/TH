...
extern BOOL 	g_bPressRunIO ;
extern BOOL		g_bPressStopIO ;
WORK_EXT    BOOL		g_bEditEnable;
WORK_EXT 	FP32		g_fStepSpeed[4];

WORK_EXT	BOOL		g_bDuxianIO;
		...
		if(g_iWorkStatus==NORUN  && g_bHandMove)
		{
			if(g_iMoveAxis<7 && g_iMoveAxis!=0)  								//借用此任务处理手摇轮
			{
				...
				if(number>=3)																		//end if(Count>g_Ini.iHandRunSpeed)
				{
					...
					if(g_iMoveAxis==2 && g_Ini.iAxisRunMode[g_iMoveAxis-1]==3 && g_Ini.iAxisSwitch[1]==1)				//送线轴
					{
						...
						if(g_Ini.iAxisSwitch[g_iMoveAxis-1]==1)
						{
							switch(g_iMoveAxis)
							{
								...
								case 2:case 3:case 4:case 5:case 6:
									if(g_Ini.iAxisRunMode[g_iMoveAxis-1]==0)
										temp=GetAngle(g_iMoveAxis,TRUE)*10;
									else if(g_Ini.iAxisRunMode[g_iMoveAxis-1]==1)			//多圈
									{
									...
									else   if (g_Ini.iAxisRunMode[g_iMoveAxis-1]==2)
									{
										temp=GetMM(g_iMoveAxis,TRUE)*100;
									}
							...
							if(fabs(temp-mv)>0.0001)
							{
								...
								if( !g_bScreen && (Line_Data+CList_StartRow() + CList_CurRow() )->value[0][0] == 'M' )
								{
									if((g_iMoveAxis == 2 &&g_Ini.iAxisRunMode[1]==1)||(g_iMoveAxis == 3 &&g_Ini.iAxisRunMode[2]==1)|| (g_iMoveAxis == 4 &&g_Ini.iAxisRunMode[3]==1)|| (g_iMoveAxis == 5 &&g_Ini.iAxisRunMode[4]==1)|| (g_iMoveAxis == 6 &&g_Ini.iAxisRunMode[5]==1))
									{
				 						CEdit_SetFloat(mv,4);//多圈
									}
									else
									{
										if(g_Ini.iAxisCa==0)  //0为1:1  1为1:0.1
										{	
											if((g_iWorkNumber==2 &&g_Ini.iAxisRunMode[1]==2) || (g_iWorkNumber==3 &&g_Ini.iAxisRunMode[2]==2) || (g_iWorkNumber==4 &&g_Ini.iAxisRunMode[3]==2))
												CEdit_SetFloat(mv/100.0,2);//丝杠
											else
												CEdit_SetFloat(mv/10.0,1);//单圈
										}
										else
											CEdit_SetLong((INT32S)mv);
									}
...
//	static  BOOL    bdisptime=TRUE;	
		...
		if(key != 0xffff)
		{
			...
			if(FALSE == ExitScrProtect())
			{
				...
				switch( key )
				{
					...
					case TRUN:
						{
							if(RealIO(SONGXIANIOCONST,0,2) || (g_bRelease && RealIO(DUANXIAN,g_Ini.iDuanXianLogic,2)) || (g_Ini.iPaoXian && RealIO(PAOXIANDI,0,2))|| (RealIO(SAFEDOOR,1,2)))
							{
								Uart_Printf("缠线 == %d\r\n  断线 == %d\r\n 跑线 == %d\r\n 安全门 == %d\r\n ",RealIO(SONGXIANIOCONST,0,2),RealIO(DUANXIAN,g_Ini.iDuanXianLogic,2),RealIO(PAOXIANDI,0,2),(RealIO(SAFEDOOR,1,2)));
								OSQPost(MsgQueue,g_bEnglish?"Start Auto Run...":"RUNTASK TRUN");
								continue;
							}
					...
					case THANDBOX:										//各轴手盒	
					{
						...
						//	g_bStepIO = TRUE;
							...
							else
							{
								...
								LedOut(BOXLED,g_bHandset==1?1:0);
	...
	//*******************外部启动加工***********************//	
		...
		// ***************外部单条*****************
		if(!g_bPressStopIO && RealIO(STOPID1,0,3)  && g_iWorkStatus==RUNNING  )
		{
			g_bPressStopIO=TRUE;
	...
	//	if(g_bWheel)
			...
			if( !g_bBackZero && g_iErrorNo ==0)
			{
				...
				if(Count > g_Ini.iHandRunSpeed*3)
				{
					...
					if(pos>0 )										
					{
						...
						{
							g_fSpeedBi=CurSpeedBi;
							SetRange((INT32S)(g_fSpeedBi*8000*g_fStepSpeed[g_iSpeedBeilv]));
						}	
					}
					else if(GetRange()!=0)
					{
		...
		{
			INT16S iRunLine  = (((vm_getindex()%500)+1)/2);//(((vm_getindex()%500)+1)/2)-1;

			if(iRunLine != g_iRunLine)
			{			
				g_iRunLine =iRunLine;
				OSQFlush(RunLineQueue);								// Clear queue
				OSQPost(RunLineQueue,(void *)&g_iRunLine);
		...
		if(RealIO(STOPIO,g_Ini.StopLevel,2)||(g_bHandset?RealIO(STOPIO2,0,3):0))			//停止
		{
			if(g_bBackZero)
			{
				switch(g_Ini.iAxisCount)
...
//					Uart_Printf("g_iErrorNo202=%d\n",g_iErrorNo);
		...
		if(g_bRelease)
		{
			if( RealIO(DUANXIAN,g_Ini.iDuanXianLogic,2)  && g_iErrorNo!=2)
			{
				g_iErrorNo=2;
		...
		if( RealIO(SONGXIANIOCONST,IOPRESS,2)&& g_iErrorNo!=3)
		{
			g_iErrorNo=3;
			SetRange(0);
		...
		{
			if(RealIO(SAFEDOOR,1,2)&& g_iErrorNo!=5 )
			{
				g_iErrorNo=5;
				SetRange(0);
		...
		//*******************检测堵线信号***********************//
		if(g_Ini.iDuXianNum && !g_bDuxianIO && RealIO(DUXIAN,0,2))
		{
			g_bDuxianIO = TRUE;
			Uart_Printf("g_bDuxianIO == %d  \r\n",g_bDuxianIO);
		}
		...
		if(g_bMode && !g_bHandRun&&(g_iWorkStatus==RUNNING||g_iWorkStatus==SINGLERUN) && g_iErrorNo ==0) 
		{
			...
			//	Uart_Printf("ComData_Taskg_iADValue=%d\n",g_iADValue);
				...
				else
				{
					if(temp<127)
					{
						INT32S AD = (INT32S)(8000*(FP32)temp/126.0*g_fStepSpeed[g_iSpeedBeilv]);	
						SetRange(AD);
