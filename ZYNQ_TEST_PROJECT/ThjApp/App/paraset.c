...
//	INT16S  j=g_iDlgTop+50-PARAHOFFSET;
	...
	{
		strcpy(ParaName,g_bEnglish?"ScrProtect(1-Yes,0-No)":"是否屏保 (1:屏保,0:不屏保)");
	 	...
	 	strcpy(ParaName,g_bEnglish?"BmpProtect(1-Yes,0-No)":"图片屏保 (1:是,0:否)");
	 	CParaSet_PutPara(ParaName," "	,&g_Ini.iScrProtectBmp		,PARAINT,0,1,0);
	 	
	 	strcpy(ParaName,g_bEnglish?"DispRunTime(1-Yes,0-No)":"显示剩余时间 (1:是,0:否)");
	 	CParaSet_PutPara(ParaName," "	,&g_Ini.iDispLeavingTime	,PARAINT,0,1,0);
	 	
 		strcpy(ParaName,g_bEnglish?"DeliverDrv(1-Yes,0-No)":"线架驱动 (1:驱动,0:不驱动)");
 		CParaSet_PutPara(ParaName," "	,&g_Ini.iXianjiaDrive		,PARAINT,0,1,0);
	 	
	 	strcpy(ParaName,g_bEnglish?"WarningOut(1-Yes,0-No)":"报警输出 (1:输出,0:不输出)");
	 	CParaSet_PutPara(ParaName," "	,&g_Ini.iWarningOut			,PARAINT,0,1,0);
	 	
	 	strcpy(ParaName,g_bEnglish?"ExceedCheck(1-Yes,0-No)":"跑线检测 (1:检测,0:不检测)");
