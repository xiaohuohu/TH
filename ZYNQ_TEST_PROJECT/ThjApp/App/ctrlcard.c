...
BOOL  InitBoard()
{
	...
	for(j=0; j<CHMAX; j++)
	{
		...
		if(j<MAXCH)//注意两个最大轴数
		{
			if(g_Ini.iPhyAxis[j]>0)
			{
				SetPhyAxis(j+1,g_Ini.iPhyAxis[j]);
			}
		}
...
//	int status=-1;
	...
	p=(INT32S)(pos*g_fPulseOfMM[ch-1]);
	Ypos = -(INT32S)(p*((FP32)(g_Ini.lRotatorRate)/1000000.0));
	
	if(ch==4 && g_Ini.lRotatorRate>0)
		inp_move4(0,Ypos,0,p);
	else
		pmove(ch,p);
	//pmove(ch,p);
}
...
//				set_actual_pos(ch,0);
		...
		if(RealIO(STOPIO,0,3)||RealIO(EXT_SINGLEDI,0,3))
		{
			Stop1(ch);
			break;
		}
...
BOOL	TestBackZero(INT16S ch,INT16S dir)
{
	...
	while( TRUE )
	{
	   ...
	   OSTimeDly(2);
		...
		if(RealIO(STOPIO,0,3)||RealIO(EXT_SINGLEDI,0,3)||(g_bHandset?RealIO(STOPIO2,0,3):0)||(g_bHandset?RealIO(SINGLEIO,0,3):0))
		{
			Stop1(ch);
			break;
		}
...
INT16S CheckWarningNoSafeDoor(void)
{
	...
	if( RealIO(DUANXIAN,g_Ini.iIOBreakLineConfig_Level,16))
	{
	...
	/*if(g_Ini.iSafeDoor>0)
	{
		if(RealIO(I_SAFEDOOR,g_Ini.iSafeDoor==1?0:1,2))//g_Ini.iSafeDoor=1为常开，2为常闭
		{
			...
			OSQPost(MsgQueue,g_bEnglish?"Safe door Waring":"安全门报警!!");
			return 13;
		}
...
{
	//  set_sub_axis((INT8U)(phy_axis),(INT8U)(pro_axis));

	  set_phy_axis((INT8U)(pro_axis),(INT8U)(phy_axis));
}
