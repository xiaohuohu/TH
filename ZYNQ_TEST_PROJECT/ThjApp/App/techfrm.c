...
//*****************835B列表*************************//
//INT16S		Widths2[COLS2]={ 60,45,105,105,120,70,45,45,101,101};
//INT16S		Widths3[COLS3]={ 60,40,88,88,100,90,67,40,40,92,92};
//INT16S		Widths4[COLS4]={ 47,36,83,83,90,80,80,67,34,34,82,81};
...
INT16S  	Widths2[COLS2]={ 52,55,110,110,118,80,76,121,71};
INT16S  	Widths3[COLS3]={ 52,55,100,100,99,90,70,50,106,71};
INT16S  	Widths4[COLS4]={ 52,52,77,77,96,86,86,50,48,101,68};
INT16S		Widths5[COLS5]={ 53,53,73,73,86,73,73,73,48,50,91,50};
INT16S		Widths6[COLS6]={ 48,31,73,73,86,73,65,70,70,48,25,85,49};//c原25

//INT16S		Widths5[COLS5]={ 53,53,80,80,91,80,80,80,54,54,91};
//INT16S		Widths6[COLS6]={ 48,31,78,78,91,78,78,78,78,48,25,85};//c原25
...
CRect		g_EditRect[MAXCH]={{110,42,120+42,42+28},{305,42,310+86,42+28},{517,42,517+43,42+28},{118,474,118+24,474+29}};

//20130819
//CRect		g_StepEditRect[MAXCH]={{118,474,118+45,474+28},{293,474,293+45,474+28},{463,474,463+45,474+28},{633,474,633+45,474+28}};
//CRect		g_StepEditRect[MAXCH]={{128,474,118+45,474+28},{303,474,293+45,474+28},{473,474,463+45,474+28},{643,474,633+45,474+28},{606,474,606+31,474+28},{728,474,728+31,474+28}};
//CRect		g_StepEditRectEx[MAXCH]={{200,474,200+36,474+28},{372,474,372+36,474+28},{540,474,540+36,474+28},{710,474,710+36,474+28},{654,474,654+19,474+28},{774,474,774+19,474+28}};
...
void	CTeach_Create()
{
	...
	INT8S   *systemName[8]={"CNC820","CNC428","CNC335","CNC625","CNC635","CNC650","TK335"};

	//0:820,1:428,2:335,3:625,4:635,5:650,6:TK335
	...
	else
	{
		...
		{
			sprintf(buf,"v%0.2f\n",VER/100.0);
			strcat(systemBuf, "  Spring Control System");
			CDC_OutText_UTF(160,8,systemBuf,g_TextColor1,g_nBk,KZK_32);
		...
		else
		{
			...
			CDC_OutText_UTF(210,8,systemBuf,g_TextColor1,g_nBk,KZK_32);
	...
	CDC_OutText_UTF(6,getmaxy()-122,g_bEnglish?"Spd":"寸动",g_TextColor6,g_nBk,KZK_24);
	CDC_OutText_UTF(6,getmaxy()-92,g_bEnglish?"Pos:":"位置",g_TextColor6,g_nBk,KZK_24);

	CDC_OutText_UTF(g_bEnglish?84:64,getmaxy()-122,g_bEnglish?"X:":axisMsg(1,":",FALSE),g_TextColor1,g_nBk,KZK_24);
	CDC_OutText_UTF(152,getmaxy()-125,"X",g_nTitleBk,g_nBk,MZK_24);

	CDC_OutText_UTF(g_bEnglish?206:186,getmaxy()-122,g_bEnglish?"Y:":axisMsg(2,":",FALSE),g_TextColor1,g_nBk,KZK_24);
	CDC_OutText_UTF(274,getmaxy()-125,"X",g_nTitleBk,g_nBk,MZK_24);

	switch(g_Ini.iAxisCount)
	{
	case 6:
	{
		CDC_OutText_UTF(g_bEnglish?694:674,getmaxy()-122,g_bEnglish?"C:":axisMsg(6,":",FALSE),g_TextColor1,g_nBk,KZK_24);
		CDC_OutText_UTF(762,getmaxy()-125,"X",g_nTitleBk,g_nBk,MZK_24);
	}
	case 5:
	{
		CDC_OutText_UTF(g_bEnglish?572:552,getmaxy()-122,g_bEnglish?"B:":axisMsg(5,":",FALSE),g_TextColor1,g_nBk,KZK_24);
		CDC_OutText_UTF(640,getmaxy()-125,"X",g_nTitleBk,g_nBk,MZK_24);
	}

	case 4:
	{
		CDC_OutText_UTF(g_bEnglish?450:430,getmaxy()-122,g_bEnglish?"A:":axisMsg(4,":",FALSE),g_TextColor1,g_nBk,KZK_24);
		CDC_OutText_UTF(518,getmaxy()-125,"X",g_nTitleBk,g_nBk,MZK_24);
	}
	case 3:
	{
		CDC_OutText_UTF(g_bEnglish?328:308,getmaxy()-122,g_bEnglish?"Z:":axisMsg(3,":",FALSE),g_TextColor1,g_nBk,KZK_24);
		CDC_OutText_UTF(396,getmaxy()-125,"X",g_nTitleBk,g_nBk,MZK_24);
	}
	break;
	}
...
void	CTeach_InvidateView(BOOL init)
{
	...
	if(init)
	{
		...
		INT8S *title5[COLS5] ={"行号","指令","开始角","结束角","送线","转芯","卷取","卷取","速比","探针","气缸","延时"};
		INT8S *title6[COLS6] ={"行号","C","开始角","结束角","送线","转芯","卷取","卷取","夹耳","速比","T","气缸","延时"};
		...
		INT8S *title5e[COLS5] ={"No","C","BgX","EdX","","","","","Rat","Prb","Cylind","Dly"};
		INT8S *title6e[COLS6] ={"No","C","BgX","EdX","","","","","","Rat","P","Cylind","Dly"};
...
void	CTeach_DisplayLine( INT16S data,INT16S row,DRAWSTATUS status,INT32S qColor,INT32S bColor)
{
	...
	for(col=0; col<CList_MaxCol(); col++)
	{
		...
		/*	else if(col == 1 && ptr->value[0][0]!=NULL)
		{
			CList_GetItemRect( row, col, &rect );
		...
		case Reset:
		{
			...
			if(col == 1)
				CDC_TextSelect_24_CRect(rect,0,m_Buffer,textColor,bColor);
			else
		...
		case MoveActive:
		{
			...
			if(col == 1)
				CDC_TextSelect_24_CRect(rect,0,m_Buffer,bColor,textColor);
			else
...
void	CTeach_AddData()
{
	...
	if(!strcmp(Line_Data[size-1].value[0],"E"))
	{
		DataPut(size-1,0,"M");
	}
	DataPut(size,0,"E");
...
//BOOL CheckSecondKey(INT16U key)
//{
//	if(g_Ini.iTestFuction)
//	{
//		if(g_bMode&& !g_bHandRun&&(g_iWorkStatus==RUNNING||g_iWorkStatus==SINGLERUN) && g_iErrorNo ==0)
//		{
//			if((key==TSTOP)||(key==TTEST)||(key==THANDSWITCH)||(key==THANDBOX)
//				||(key==TXPLUS)||(key==TXMINUS)||(key==TUP)||(key==TDOWN)
//				||(key==TRIGHT)||(key==TLEFT)||(key==TENTER)||(key=='1')
//				||(key=='2')||(key=='3')||(key=='4')||(key=='5')||(key=='6')
//				||(key=='7')||(key=='8')||(key=='9')||(key=='0')||(key==TDOT)
//				||(key==TMINUS)||(key==TBACKSPACE)||(key==TINSERT)||(key==TDEL))
//			{
//				return TRUE;
//			}
//		}
...
void  Teach_Task(void *data)
{
	...
	for(;;)
	{
		...
		if(key != 0xffff)
		{
			...
			if((key>=F1&&key<=F10) ||(key==F11 || key==F12))
			{
				if(g_Ini.iHandStatus==1)
				{
...
void	CTeach_LoopCheck()
{
	...
	//************************断线、缠线、跑线检测***********************//
	{
		...
		{
			bit = ReadBit(DUANXIAN);
			if(g_bRelease)
			{
				if(bit!=OtherIOStatus[0])
				{
					OtherIOStatus[0]=bit;
					if(bit==g_Ini.iIOBreakLineConfig_Level)
					{
			...
			if(g_Ini.iSafeDoor)
			{
				bit=ReadBit(I_SAFEDOOR);

				if(bit!=OtherIOStatus[3])
				{
					OtherIOStatus[3]=bit;

					if(bit==(g_Ini.iSafeDoor==1?0:1))
					{
						OSQPost(MsgQueue,g_bEnglish?"Safety door alarm":"安全门报警!");
	...
	//*******************自动加工***********************//
	{
		...
		if(!g_bPressStartIO&&(RealIO(STARTIO,IOPRESS,3)||RealIO(EXT_STARTIO,0,2)||(RealIO(RUNIO,0,2)&&g_bHandset)))
		{
	...
	{
		if(!g_bPressStepIO2 && RealIO(SINGLEDI1,0,2) /*&&RealIO(SINGLEDI2,0,2)*/)                         //IO口单条
		{
		...
		if(g_bPressStepIO2 && RealIO(SINGLEDI1,1,2) /*&& RealIO(SINGLEDI2,1,2)*/)
		{
	...
	if((g_Ini.iHandStatus || g_bHandset) && !g_bMode )
	{
		...
		if(labs(pos)>1 )
		{
			...
			if(g_iWorkNumber == 2 && g_Ini.iYDaoXian == 0 && pos < 0)						//20141128在寸动速度编辑下可以手轮摇动
			{
...

			{
				if(g_iWorkNumber<g_Ini.iAxisCount+1 && g_iWorkNumber!=0)//自动模式下手轮移动各个轴
				{
					...
					//手轮内部补偿功能，实质在于单轴手动另外一个轴同步运动  2020.10.24 cj

					//					if(g_iWorkNumber==g_Ini.iWorkAxisNo && g_Ini.iWorkAxisTMolecular>1 /*&& g_Sysparam.fReviseFTLDenominator>0*/)
					//					{
					...
					//						INT32S Pos_Pulse0,Pos_Pulse1;
					//
					//						Pos_Pulse1=10000000L;
					//						Pos_Pulse0=-(INT32S)((FP32)Pos_Pulse1*g_Ini.iWorkAxisTMolecular/1000000.0);
					//
					//						//ps_debugout("g_iWorkNumber==%d 限位= %d\r\n",g_iWorkNumber,CheckLimit(TRUE,TRUE,g_iWorkNumber,pos));
					...
					//							if(pos>0)
					//							{
					//								inp_move2(4,g_iWorkNumber,Pos_Pulse0,Pos_Pulse1);
					//							}
					//							else
					//							{
					//								inp_move2(4,g_iWorkNumber,-Pos_Pulse0,-Pos_Pulse1);
					...
					case 2:
						...
						if(g_Ini.iYDaoXian)
						{
							...
							mv = GetMM(g_iWorkNumber,TRUE)*100;
							g_iMoveAxis = 0;
							g_bHandMove = FALSE;
						}
						else
					...
					case 4:
						...
						if(g_Ini.lRotatorRate>0)
						{
							Ypos = Apos>0?-(INT32S)(Apos*(FP32)g_Ini.lRotatorRate/1000000.0):-(INT32S)(Apos*(FP32)g_Ini.lRotatorRate/1000000.0);

							ShortlyInp_Move2(Ypos, Apos, speed);
						}
						else
							ShortlyPmove(g_iWorkNumber,pos>0?10000000L:-10000000L,speed);
					...
					{
						if((g_iWorkNumber == 3 &&g_Ini.iAxisRunMode[2]==1)|| (g_iWorkNumber == 4 &&g_Ini.iAxisRunMode[3]==1)|| (g_iWorkNumber == 5 &&g_Ini.iAxisRunMode[4]==1)|| (g_iWorkNumber == 6 &&g_Ini.iAxisRunMode[5]==1))
						{
							CEdit_SetFloat(mv,4);
						}
						else
	...
	if(g_uiMoveKey !=0 && !g_bMode /*&& !g_bWheel*/ && g_Ini.iHandStatus==0 && !g_bHandset)
	{
		...
		case F1:
			...
			{
				if(g_iCurEditNo==9 && CList_CurCol()!=2 && CList_CurCol()!=3)
				{
					CTeach_Update(0,2-CList_CurCol());
				}
		...
		case F3:
			...
			{
				//g_iWorkNumber=0;
				OSQPost(MsgQueue,axisMsg(2,g_bEnglish?" is Turned off.Can't Move!":"未开启,寸动功能不可用!",TRUE));
				//break;
				goto PRESSKEYEND ;
			}
			else
			...
			if(g_iCurEditNo==9 && CList_CurCol()!=4)
			{
				CTeach_Update(0,4-CList_CurCol());
				UpdateCurAxis();
			}
		...
		case F5:
			if(g_Ini.iAxisSwitch[2]==0||g_Ini.iAxisCount<3)
			{
				...
				OSQPost(MsgQueue,axisMsg(3,g_bEnglish?" is Turned off.Can't Move!":"未开启,寸动功能不可用!",TRUE));
				goto PRESSKEYEND ;
			}
			else
				OSQPost(MsgQueue,"");

			if(g_iCurEditNo==9 && CList_CurCol()!=5)
			{
				CTeach_Update(0,5-CList_CurCol());
				UpdateCurAxis();
			}
		...
		case F7:
			...
			if(g_Ini.iAxisSwitch[3]==0)
			{
				...
				OSQPost(MsgQueue,axisMsg(4,g_bEnglish?" is Turned off.Can't Move!":"未开启,寸动功能不可用!",TRUE));
				goto PRESSKEYEND ;
			}
			else
				OSQPost(MsgQueue,"");

			if(g_iCurEditNo==9 && CList_CurCol()!=6)
			{
				CTeach_Update(0,6-CList_CurCol());
				UpdateCurAxis();
			}
		...
		case F9:
			...
			if(g_Ini.iAxisSwitch[4]==0)
			{
				...
				OSQPost(MsgQueue,axisMsg(5,g_bEnglish?" is Turned off.Can't Move!":"未开启,寸动功能不可用!",TRUE));
				goto PRESSKEYEND ;
			}
			else
				OSQPost(MsgQueue,"");

			if(g_iCurEditNo==9 && CList_CurCol()!=7)
			{
				CTeach_Update(0,7-CList_CurCol());
				UpdateCurAxis();
			}
		...
		case F10:
			...
			//Uart_Printf("g_iWorkNumber11 =%d\n",g_iWorkNumber);
			PressKeyMove(5,7,0);
			//Uart_Printf("g_iWorkNumber12 =%d\n",g_iWorkNumber);
		...
		case F11:
			...
			if(g_Ini.iAxisSwitch[5]==0)
			{
				...
				OSQPost(MsgQueue,axisMsg(6,g_bEnglish?" is Turned off.Can't Move!":"未开启,寸动功能不可用!",TRUE));
				goto PRESSKEYEND ;
			}
			else
				OSQPost(MsgQueue,"");

			if(g_iCurEditNo==9 && CList_CurCol()!=8)
			{
				CTeach_Update(0,8-CList_CurCol());
				UpdateCurAxis();
			}
	...
	if(g_Ini.iIOSendWire==1 && !g_bMode  && g_Ini.iHandStatus==0)
	{
		INT16S ch = 2;
		INT16S dir =0;
		FP32 Speed=10.0;
		...
		/*	if(g_bUseTimes)
			Speed = g_Ini.lHandSpeed[ch-1]*g_Ini.iStepRate[ch-1]/10;
		else
		...
		if(RealIO(SONGXIAN,0,2))
		{
			...
			AddMoveMM(ch,dir?10000:-10000,1.0,Speed,g_Ini.lHandSpeed[ch-1]*25);

			while(TRUE)
			{
				if(RealIO(SONGXIAN,1,2))
					break;
	...
	if(g_Ini.iIOSendWire==1 && !g_bMode  && g_Ini.iHandStatus==0)
	{
		...
		if(RealIO(AHANDADDDI,0,2))
		{
			...
			AddMoveMM(ch,dir?10000:-10000,1.0,Speed,g_Ini.lHandSpeed[ch-1]*10);

			while(TRUE)
			{
				if(RealIO(AHANDADDDI,1,2))
					break;
...
void	CTeach_OnKey(INT16U key,INT16U tkey)
{
	...
	//if(pv) while(1){OSTimeDly(1);}
	//pv=true;


	if (!g_bEditEnable && g_Ini.iPasswordTime!=0)  //编辑数据需要密码
	{
		if ((key>='0' && key<='9') || key==TDOT || key==TSAVE || key==TDEL ||  key=='-'
				|| key==TINSERT || key==TDELALL  || key==TBACKSPACE ||key==TSTARTINPUT||key==TENDINPUT)
		{
	...
	if(g_iCurEditNo!=9)
	{
		...
		if(TBACKZERO == key || TSTOP ==key || TTEST ==key || TSCOMMAND ==key || TGCOMMAND ==key || TMCOMMAND ==key ||TJCOMMAND ==key || TECOMMAND==key
				|| TLCOMMAND==key ||TNCOMMAND ==key ||TPAGEDOWN ==key ||TPAGEUP ==key || TENDINPUT==key || TSTARTINPUT ==key ||TDEL ==key ||TINSERT ==key || TDELALL ==key
				|| (key>=F1 && key<=F8) )
		{
			UpdateFileDestProbe(g_iCurEditNo+1,1,0);
			g_iCurEditNo = 9;
	...
	case TBACKZERO:										//归零键
	{
		...
		else
		{
			...
			/*if(CTeach_BackZero(1))
			{
				...
				while(ch<=g_Ini.iAxisCount)
				{
					if(g_Ini.iAxisSwitch[ch-1]==1)
					{
						if(!CTeach_BackZero(ch))				//第N轴回零失败
						{
							SuspendTasks(1);
							OSTimeDly(50);
							ResumeTasks(0);
	...
	case TUSWITCH:										//卷取
	{
		...
		/*if(g_Ini.iAxisSwitch[3]==0)
					g_lAxisValue[3]=GetPulse(4,TRUE);
				else
					SetPulse(4,g_lAxisValue[3],TRUE);*/
	...
	case TVSWITCH:										//卷取2
	{
		if(g_Ini.iAxisCount<5)
			break;
		//Uart_Printf("g_Ini.iAxisSwitch[4]=%d\n",g_Ini.iAxisSwitch[4]);
		g_Ini.iAxisSwitch[4]=g_Ini.iAxisSwitch[4]==1?0:1;
		//Uart_Printf("g_Ini.iAxisSwitch[4]=%d\n",g_Ini.iAxisSwitch[4]);

		/*if(g_Ini.iAxisSwitch[4]==0)
					g_lAxisValue[4]=GetPulse(5,TRUE);
				else
					SetPulse(5,g_lAxisValue[4],TRUE);*/

		LedOut(VSWITCHLED,g_Ini.iAxisSwitch[4]?0:1);
	...
	case TWSWITCH:										//夹耳
	{
		...
		g_Ini.iAxisSwitch[5]=g_Ini.iAxisSwitch[5]==1?0:1;

		/*if(g_Ini.iAxisSwitch[5]==0)
					g_lAxisValue[5]=GetPulse(6,TRUE);
				else
					SetPulse(6,g_lAxisValue[5],TRUE);*/

		LedOut(WSWITCHLED,g_Ini.iAxisSwitch[5]?0:1);;
	...
	{
		//	if( CheckWarning()	|| CheckLimit(TRUE,TRUE,0,0))
		if( CheckWarningNoSafeDoor()|| CheckLimit(TRUE,TRUE,0,0))
		{
			break;
		}
		...
		if(g_Ini.iCyldOrOrigin == 0)//气缸测试按键
		{
		...
		else				//设置起点按键
		{
			...
			case 4:
			{
				...
				{
					OSQPost(MsgQueue,g_bEnglish?"Please Reset A":"A轴先归零 ...");
					return;
				}
				else
				{
			...
			case 3:
			{
				...
				{
					OSQPost(MsgQueue,g_bEnglish?"Please Reset Z":"Z轴先归零 ...");
					return;
				}
				else
				{
			...
			case 1:
			{
				...
				{
					OSQPost(MsgQueue,g_bEnglish?"Please Reset X":"X轴先归零 ...");
					return;
				}
				else
				{
...
//		if(g_Ini.iTestFuction&&Check_GoZero())
//		{//回完零后才设置上特殊测试标志位
//			g_bBardTest = TRUE;
//		}
	...
	{
		if(CList_StartRow() + CList_CurRow() == 0)
		{
			OSQPost(MsgQueue,g_bEnglish?"command M format error!":"M指令数据格式错误!");
			break;
		}
...

	{
		...
		for(a=5;a<8;a++)
			SuspendTasks(a);
		CDC_SaveScreenXY(250,140,225,155);
		OSTimeDly(50);
		if(g_bEnglish)
			CMsgDlg_Init(250,140,220,150,"Insert a instruction?","warnning",REDCOLOR,"Yes","No");
		else
			CMsgDlg_Init(250,140,220,150,"插入一行指令吗?","警告",REDCOLOR,"是","否");
		...
		CMsgDlg_Destroy();
		CDC_RestoreScreenXY(250,140,225,155);
	...
	case TDEL:												//删除单行
	{
		...
		OSTimeDly(50);
		if(g_bEnglish)
			CMsgDlg_Init(250,140,220,150,"Deletes row data?","warnning",REDCOLOR,"Yes","No");
		else
			CMsgDlg_Init(250,140,220,150,"删除当前行数据吗?","警告",REDCOLOR,"是","否");
		...
		if(g_iCurEditNo==0)
		{
			...
			if(tmpfile!=tmpfile2)
			{
				//if(FileExist(tmpfile2))
				{
					...
					OSTimeDly(50);
					CDC_SaveScreenXY(200,140,360,190);
					...
					if(CMsgDlg_DoModal())
					{
						...
						for(a=5;a<8;a++)
							ResumeTasks(a);
						CDC_RestoreScreenXY(200,140,360,190);
	...
	{
		{
...

			{
				...
				else if( CList_CurCol() == 4 )					//当在送线轴编辑框时按ENTER键时
				{

					if(g_iWorkStatus==NORUN&&g_lStepNode==0)
					{
						SetMM(2,0,TRUE);
						g_bFirstResetY=FALSE;
						DisplayXYZ(TRUE);
	...
	case '0':												//0.-清楚键输入
		...
		/*
			if(g_bCylinder) //20070810 add by fzz
			{
				int a;

				if(PressKeyTime(1500))
				{
					...
					CMsgDlg_Init(210,140,300,120,g_bEnglish?"Upadate system file?":"确实要升级系统文件?",g_bEnglish?"Warning":"警告",REDCOLOR,
					...
					if(CMsgDlg_DoModal())
					{
						...
						case -1:
							...
							break;
						case -2:
							OSQPost(MsgQueue,g_bEnglish?"U Disk System update failed!":"U盘程序升级失败,请确定U盘读写正常!");
							break;
						case -3:
	...
	case TBACKSPACE:										//清除命令按键
	{
		...
		OSTimeDly(50);
		if(g_bEnglish)
			CMsgDlg_Init(250,140,220,150,"Clear current data?","warnning",REDCOLOR,"Yes","No");
		else
			CMsgDlg_Init(250,140,220,150,"清除当前数据吗?","警告",REDCOLOR,"是","否");

		if(CMsgDlg_DoModal())
		{
			CMsgDlg_Destroy();
			CDC_RestoreScreenXY(250,140,225,155);
			CTeach_Update(0,0);//再次定位当前光标，防止密码验证之后，清除位置错误
			CEdit_OnKey( key, tkey );
	...
	case F2:
		if(g_bHandset)
		{
			if(ReadBit(TXIO)==0 || ReadBit(TYIO)==0 || ReadBit(TZIO)==0 || (g_Ini.iAxisCount>3)&&ReadBit(TAIO)==0 || (g_Ini.iAxisCount>4)&&ReadBit(TBIO)==0|| (g_Ini.iAxisCount>5)&&ReadBit(TCIO)==0)
			{
				OSQPost(MsgQueue,g_bEnglish?"This key is invalid at state of handset":"手持盒操作状态该键无效!");
				break;
			}
	...
	case TSTARTINPUT:										//开始键
	{
		char str[0x20]="\0";
		char temp[0x20];
		//INT32S curPos = 0;
		int row = CList_CurRow()+CList_StartRow();
		COMMAND *ptr = Line_Data+row;

		g_bEditData=FALSE;
		g_bModify = TRUE;
		g_bModifySave=FALSE;

		//curPos = (INT32S)(GetAngle(1,TRUE)*10);

		//ltoa(curPos,(INT8U *)str);
		ltoa((INT32S)(GetAngle(1,TRUE)*10),(INT8U *)str);

		strcpy(temp,ptr->value[1]);
		strcpy(ptr->value[1],str);

		if( !CTeach_ProcessError( ptr, 2, str) )
		{
			strcpy(ptr->value[1],temp);
			break;
		}

		{
			...
			CList_GetItemRect(CList_CurRow(), 2, &rect );

			CDC_PutColorBar2( rect,g_nTableBk,1);
			CDC_Rectangle2( rect ,g_TableLine);

			CTeach_ViewConvert( CList_CurRow(), 2, pBuffer );

			CDC_TextSelect_24(rect.left,rect.top,rect.right-rect.left,rect.bottom-rect.top,1,pBuffer,g_TextColor1,-1);

			//if(CList_CurCol()==2||CList_CurCol()==3)
			CTeach_Update(0,3-CList_CurCol());          //修正“开始”按键的行为，在按“开始”按键时不让光标跑到其他轴上。
		}

		//g_lInputPos[0] = curPos;
		WriteDataToFile( g_Ini.iFileName );
		DisplayXYZ(TRUE);
		UpdateCurAxis();
	}
	...
	case TENDINPUT:											//结束按键
	{
		...
		strcpy(temp,ptr->value[2]);

		strcpy(ptr->value[2],str);

		if( !CTeach_ProcessError( ptr, 3, str) )
		{
			strcpy(ptr->value[2],temp);
			break;
		}

		g_lInputPos[0] = curPos;

		if(g_Ini.iAxisSwitch[1]==1&&GetPulse(2,TRUE)!=0)
		{
			ltoa((INT32S)(GetMM(2,TRUE)*100),(INT8U *)str);

			strcpy(temp,ptr->value[3]);
			strcpy(ptr->value[3],str);

			if( !CTeach_ProcessError( ptr, 4, str) )
			{
				strcpy(ptr->value[3],temp);
				break;
			}
		...
		{
		case 6:
			if(g_Ini.iAxisSwitch[5]==0 || (g_Ini.iAxisRunMode[5] == 1 && g_Ini.iAxisMulRunMode[5]==0))//多圈相对模块下不能教导
			{
			}
			else
			{
				if (g_Ini.iAxisRunMode[5]==0)
					ltoa((INT32S)(GetAngle(6,TRUE)*10),(INT8U *)str);
				else if (g_Ini.iAxisRunMode[5]==1)
				{
					...
					ftoa( mv,(INT8U *)str,4,0);
				}
				else if(g_Ini.iAxisRunMode[5] == 2)
				{
					curPos = (INT32S)(GetMM(6,TRUE)*100);
					ltoa(curPos,(INT8U *)str);
				}

				//if(g_Ini.iAxisRunMode[5] != 1)
				{
					strcpy(temp,ptr->value [7]);
					strcpy(ptr->value[7],str);

					if( !CTeach_ProcessError( ptr, 8, str) )
					{
						strcpy(ptr->value[7],temp);
						return;
					}
				}
			}
		...
		case 5:
			if(g_Ini.iAxisSwitch[4]==0 || (g_Ini.iAxisRunMode[4] == 1&& g_Ini.iAxisMulRunMode[4]==0))
			{
			}
			else
			{
				if (g_Ini.iAxisRunMode[4]==0)
					ltoa((INT32S)(GetAngle(5,TRUE)*10),(INT8U *)str);
				else if (g_Ini.iAxisRunMode[4]==1)
				{
					INT16S  tempi1;
					FP32 	tempi2;
				...
				{
					curPos = (INT32S)(GetMM(5,TRUE)*100);
					ltoa(curPos,(INT8U *)str);
				}

				strcpy(temp,ptr->value [6]);
				strcpy(ptr->value[6],str);

				if( !CTeach_ProcessError( ptr, 7, str) )
				{
					strcpy(ptr->value[6],temp);
					return;
				}
			}

		case 4:
			if(g_Ini.iAxisSwitch[3]==0 || (g_Ini.iAxisRunMode[3] == 1&& g_Ini.iAxisMulRunMode[3]==0))
			{
			...
			{
				if (g_Ini.iAxisRunMode[3]==0)
					ltoa((INT32S)(GetAngle(4,TRUE)*10),(INT8U *)str);
				else if (g_Ini.iAxisRunMode[3]==1)
				{
					INT16S  tempi1;
					FP32 	tempi2;
				...
				{
					curPos = (INT32S)(GetMM(4,TRUE)*100);
					ltoa(curPos,(INT8U *)str);
				}

				strcpy(temp,ptr->value [5]);
				strcpy(ptr->value[5],str);

				if( !CTeach_ProcessError( ptr, 6, str) )
				{
					strcpy(ptr->value[5],temp);
					return;
				}
			}

		case 3:
			if(g_Ini.iAxisSwitch[2]==0 || (g_Ini.iAxisRunMode[2] == 1&& g_Ini.iAxisMulRunMode[2]==0))
			{
			...
			else
			{
				if(g_Ini.iAxisRunMode[2] == 0)
				{
					curPos = (INT32S)(GetAngle(3,TRUE)*10);
					ltoa(curPos,(INT8U *)str);
				}
				else if(g_Ini.iAxisRunMode[2] == 1)
				{
				...
				{
					curPos = (INT32S)(GetMM(3,TRUE)*100);
					ltoa(curPos,(INT8U *)str);
				}

				//if(g_lInputPos[2]!=curPos)
				//if(g_Ini.iAxisRunMode[2] != 1)

				strcpy(temp,ptr->value[4]);
				strcpy(ptr->value[4],str);

				if( !CTeach_ProcessError( ptr, 5, str) )
				{
					strcpy(ptr->value[4],temp);
					break;
				}
				//g_lInputPos[2] = curPos;
			}
			break;
...

		{
			...
			for(l=3;l<g_Ini.iAxisCount+3;l++)
			{
				CList_GetItemRect( CList_CurRow(), l, &rect );
				CDC_PutColorBar2( rect,g_nTableBk,1);
				CDC_Rectangle2( rect ,g_TableLine);
				CTeach_ViewConvert( CList_CurRow(), l, pBuffer );
				CDC_TextSelect_24(rect.left,rect.top,rect.right-rect.left,rect.bottom-rect.top,1,pBuffer,g_TextColor1,-1);
				//CDC_OutText( rect.left+2,rect.top+5, pBuffer, g_TextColor1,-1);
			}
			...
			if( row == Line_Data_Size-1 )
			{
...

				{
					...
					CEdit_SetFloat(GetAngle(1,TRUE)*10,0);
	...
	case 'M':
	{
		...
		if(g_Ini.iAxisCount>=3)
		{
			if(col==5)
			{
				...
				{
					if( n>3600.01 || n < -3600.01) //20070927 modify
					{
						OSQPost(MsgQueue,g_bEnglish?"3600>=angle>=-3600":"3600>=角度>=-3600");
						return FALSE;
					}
...
Uart_Printf("速比：%d\r\n",n);
		...
		if(col==CList_MaxCol()-3)
		{
			...
			//if(g_Ini.iAxisCount>2)
			{
				...
				{
					OSQPost(MsgQueue,g_bEnglish?"probe <=7 if Exceed Wire Check.":"有跑线检测时,探针设定<=7");
					return FALSE;
				}
				else
				{
					...
					{
						OSQPost(MsgQueue,g_bEnglish?"Probe <=8":"探针设定<=8");
						return FALSE;
					}
				}
			}
			/*else
				{
					if(g_Ini.iPaoXian&&n>3.01)
					{
						OSQPost(MsgQueue,g_bEnglish?"probe <=3 if Exceed Wire Check.":"有跑线检测时,探针设定<=3");
						return FALSE;
					}
					else
					{
						if(!g_Ini.iPaoXian&&n>4.01)
						{
		...
		if(col==CList_MaxCol()-2)
		{
			...
			if(g_Ini.iCyldMode==0)
			{
				...
				{
					if((string[i]<'0'||string[i]>'2') && string[i]!=NULL)
					{
						OSQPost(MsgQueue,g_bEnglish?"Data Invalid,Make Sure it's 01 Sequence.":"气缸设定无效,确保为0-2序列");
						return FALSE;
					}
				}
			}
		...
		if(col==CList_MaxCol()-1 && string[0]!=NULL)
		{
			if( n>600.0 || n<0.0)
			{
				OSQPost(MsgQueue,g_bEnglish?"600.0>Delay>0.0":"600.0>=延时>=0.0");
				return FALSE;
			}
		}
	...
	while(TRUE)
	{
		...
		if(key != 0xffff)
		{
			...
			case '8':  														//公司级系统管理密码设定
			{
				...
				CPassword_Init(200,140,400,220,g_bEnglish?"Company System Pwd":"公司级系统管理密码验证",buf,g_TextColor7);
				...
				if(CPassword_DoModal())
				{
					...
					if(/*pass==NewDog.Adtech_Password ||*/ password == NewDog.Adtech_ComputerNo ||pass==116001)
					{

						CSetDog_Init(180,140,420,230);
						CSetDog_DoModal();
						CSetDog_Destroy();
					}
				}
