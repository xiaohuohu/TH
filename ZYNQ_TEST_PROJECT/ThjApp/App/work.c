...
BOOL		g_bDuxianIO = FALSE;					//堵线信号检测标志位
INT32S		nLastDuxCount=0;						//堵线计数
	...
	{	
		...
		if( g_Ini.iAxisSwitch[1]==1 )
		{
			...
			else if(g_Ini.iAxisRunMode[1] == 1)
			{
				...
				curPos[1] = (INT32S)(GetMM(2,cur)*10);//取除系统坐标
			...
			else if(g_Ini.iAxisRunMode[1] ==3)		//送线
			{
				Uart_Printf("送线...\n");	
				curPos[1] = (INT32S)(GetMM(2,cur)*100);
				curPos[1] = (curPos[1] -  GetYLong(vm_getindex()) - GetMMFromPulse(2,lYProbelong*100));
...
INT16S	RunAllData(void)
{
	...
	BOOL		openpro ;//记录当前教导界面是否有探针设置为1
	...
	if(g_bRelease)
	{
		...
		case 2:
			if(g_Ini.iAxisSwitch[1]==1)
			{
				if( !g_bFirstResetY  )
				{
					OSQPost(MsgQueue,g_bEnglish?"Please Reset Y!!!":"Y先归零 ...!!!");
					return 1;
				}
			}
	...
	{															//启动前若以归过机械原点，但现在不在零点需回零点
		...
		if(g_Ini.iAxisSwitch[1]==1)
		{
			...
			if(g_Ini.iBackMode[1]!=4  || g_Ini.iAxisRunMode[1]==1)	//多圈轴都要先回零,即使错误设置成不回零
			{
				if(lTemp!=0)
				{
					if(!WorkFindZero(2))
		...
		for(; lNode<g_lDataCopyXmsSize;)						//加工一条弹簧的数据发送
		{
			...
			case MOVE:
			{
...

				{
					 ...
					 */
					//Uart_Printf("test\n");
					//一次性发最多发八条数据 如果中间有探针气缸等待,会等待完成在继续缓存
...
#if FALSE
				if( pData.nIO>0&&pData.nIO<2) 												//当发送到探针行时停止发送缓存的数据，并等待探针信号
				{
						...
						if(pData.nLine == vm_getindex())				//等待执行缓存到探针行。注：探针行的行号与开始角行号一致。
						{
							...
							if(bFirstProbe)								//打开探针,并首次检查探针状态若有信号探针出错
							{
								...
								case 1:
								{
									...
									if(flag)
										g_lProbeAngle = -188888.0;//探针1前触碰错误提示
								...
								if(flag ==1)							//表示缓存中探针以打开，并首次就检测到探针信号
								{
									...
									if(g_Ini.lProbeCount ==1)
									{
										...
										OSQPost(MsgQueue,g_bEnglish?"Probe keep touching!":"探针一直触碰,按\"启动\"或\"测试\"继续");

										while(TRUE)						//此处如果有2.3.4.17这几个报警，程序会处于while循环中
										{
							...
							else										//检测到探针信号
							{
								...
								if(flag ==1 )							//探针成功
								{
									...
									lYProbelong1+=lYProbelong;
									SetPulse(2,pData.lValue[1],TRUE);   //把最后值直接设置进去,让系统认为已经达到了结束坐标
									vm_sync();							//同步，清空缓存
									//CloseProbe(0);
									break;								//退出检测段执行下一行?
								}
						...
						if (pMQ->QueueCount==0 && ReadFifo()==0 && IsInpEnd(1))	//若探针行执行完。
						{
							...
							else
							{
								...
								if(g_Ini.lProbeCount ==1)
								{
									...
									OSQPost(MsgQueue,g_bEnglish?"Probe hasn't touched!":"探针未触碰,按\"启动\"或\"测试\"继续\n");
					...
					SetCommandPos(1,0);				 					//探针检测后清除位置，便于脉冲滞后检测
					SetActualPos(1,0);
					g_bLagCheck = TRUE;
																...
																//探针8用作安全门
				if(pData.fDelay-0.0>0.000001 ||  ( pData.nIO>0 && pData.nIO!=8 && g_Ini.iProDelay>0))					//输入或延时
				{
					INT32U tt2=0;
					INT32U tt1=0;
					...
					while(TRUE)					//等待缓存数据发完再开始延时,或IO等待
					{
					...
					if(g_Ini.iProDelay>0 && pData.nIO)							//输入等待
					{
						...
						while(TRUE)
						{
							...
							//if((tt2-tt1) >= (INT32U)(10 *1000.0))//输入等待10秒超时
				...
				}// END 	if(pData.fDelay-0.0>0.000001 ||  pData.nIO>0)					//输入或延时
						...
						if(lTagCount>=TAGBUFCOUNT)
						{
							...
							while(TRUE)
							{
								...
								else if((vm_get_tag()+TAGBUFCOUNT)>lTagLastCount)
								{
									...
									if(g_Ini.lRunCount>=g_Ini.lDestCount-1)
									{
										//OSQPost(MsgQueue,g_bEnglish?"Count of running arrive!":"加工数量到达!");
										g_bAutoRunMode=FALSE;
...
void	RunEnd()
{
	...
	if(g_Ini.lRunCount>=g_Ini.lDestCount)						//加工完成提示
	{
		OSTimeDly(70);
		...
		sprintf(g_pszMsgBuf,g_bEnglish?"%dAmount fill!":"加工数量已完成:\n     %d个!",g_Ini.lRunCount);
		CAfxMsgDlg_Init(250,200,300,150,g_pszMsgBuf,g_bEnglish?"Waring":"警告",REDCOLOR);
		CAfxMsgDlg_DoModal();
		CAfxMsgDlg_Destroy();
	...
	{															//报警提示
		...
		case 5: OSQPost(MsgQueue,g_bEnglish?"Safe Door Waring":"安全门打开状态!!!");						break;
		...
		case 60:OSQPost(MsgQueue,g_bEnglish?"IO Wait timeout alert":"输入等待超时报警!"); break;
		case 61:OSQPost(MsgQueue,g_bEnglish?"Blocking line Waring":"堵线报警!"); break;
		default:
			break;
		}

		if(g_iErrorNo>=1)			//急停和正常退出不报警
			RunStatusOutCtr(1);
		else
			RunStatusOutCtr(0);
...
//连续加工iDuXianNum个零件没有检测到堵线信号就急停报警
void	CheckDuxian()
{
	if(g_Ini.iDuXianNum)
	{
