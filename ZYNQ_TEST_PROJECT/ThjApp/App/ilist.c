...
void	CList_GetItemRect( INT16S row,INT16S col, CRect *rect )
{
	...
	rect->top 	= (row+1) * m_nListHeight + m_nListY;
...
*/
	rect.left=m_nListX;
	rect.top=m_nListY;
	rect.bottom=m_nListY + m_nListHeight;
...
//		CDC_OutCenText24( rect, tilte[j], qcolor ,bcolor);
		
		CDC_TextSelect_32(rect.left+2, rect.top, rect.right-rect.left, rect.bottom-rect.top,0,tilte[j], qcolor ,bcolor);
	
		if(SVGAmode==0)
			CDC_Rectangle2( rect,g_TextColor3 );
		else
		{
			#if DISPTYPE==0
				CDC_Rectangle2( rect,DARKGREENCOLOR );
			#else
