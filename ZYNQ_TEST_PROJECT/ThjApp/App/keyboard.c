...
void	CKeyBoard_UpdateObject(INT16S dk,BOOL bFrist)
{
	...
	if(m_iCurKey<40)
	{
		...
		CDC_OutText24Ex(x+6,y,keyName[m_iCurKey],WHITECOLOR,m_KeyBackColor);
	}
	else
	{
		y = g_iDlgTop+200;	
	...
	if(bFrist)
	{
		...
		for(i = 0 ; i < 4 ;i++)
		{
			...
			for(j = 0 ; j < 11 ;j++)
			{
				...
				CDC_OutText24Ex(x+6,y,keyName[key++],WHITECOLOR,m_KeyBackColor);
	...
	if(m_iCurKey<40)
	{
		...
		CDC_OutText24Ex(x+6,y,keyName[m_iCurKey],LIGHTREDCOLOR,m_KeyColor);
	}
	else
	{
		y = g_iDlgTop+200;	
