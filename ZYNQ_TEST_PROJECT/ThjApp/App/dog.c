...
INT32U	InitDog(void)
{
	...
	NewDog.UserDogIdentifier = 8008;											//4)	用户级软件狗标别码
	
	NewDog.User_ComputerNo = 123;												//5)	用户级客户机器编号
	...
	if(Set1)														//用户级加密狗算法流程
	{
		...
		if(NewDog.User_Days<=5)
		{
			while(1)
			{
				...
				sprintf(buf," 日期:%s  \n 软件狗识别码:%ld  \n 用户机器编号:%ld",da,NewDog.UserDogIdentifier,NewDog.User_ComputerNo);
				sprintf(bufE,"DATA:%s  \n UserNo.:%ld		\n MacSerial:%ld",da,NewDog.UserDogIdentifier,NewDog.User_ComputerNo);
			
				if(NewDog.User_Days>0)
					CPassword_Init(200,150,360,250,g_bEnglish?"Test Day End Input User Password:":"试用期快满请输入用户密码",g_bEnglish?bufE:buf,g_TextColor2);
				else
					CPassword_Init(200,150,360,250,g_bEnglish?"Test Day End Input User Password:":"试用期已满请输入用户密码",g_bEnglish?bufE:buf,g_TextColor2);
	...
	if(Set2)
	{
		...
		if(NewDog.Adtech_Days<=5)
		{
			while(1)
			{
				...
				sprintf(bufE,"DATA:%s  \n AdtechSerial:%ld",da,NewDog.User_ComputerNo);
			
				if(NewDog.Adtech_Days>0)
					CPassword_Init(200,150,360,250,g_bEnglish?"Test Day End Input User Password:":"试用期快满请输入公司级密码",g_bEnglish?bufE:buf,g_TextColor2);
				else
					CPassword_Init(200,150,360,250,g_bEnglish?"Test Day End Input User Password:":"试用期已满请输入公司级密码",g_bEnglish?bufE:buf,g_TextColor2);
